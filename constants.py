...
except ValidationError: #tmp for testing
    KEYS = Keys(_env_file='../.env', _env_file_encoding='utf-8')
...
reminder_pattern_words = np.array([r'^\d?\d:\d\d every \d+ days', r'^\d?\d:\d\d every \d+ day', r'^\d?\d:\d\d every day', r'^\d?\d:\d\d каждый день', r'^\d?\d:\d\d каждый \d+ день', r'^\d?\d:\d\d каждые \d+ дней', r'^\d?\d:\d\d', r'^в \d?\d:\d\d'])
